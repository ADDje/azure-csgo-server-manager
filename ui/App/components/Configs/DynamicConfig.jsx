...
class DynamicConfig extends React.Component {
    ...
    constructor(props) {
        ...
        if (this.props.config !== null) { 
            Object.assign(this.state.configParameters, this.props.config)
        }
        ...
        for(var key in this.props.config) {
            ...
            fields.push(<div key={key} className="dynamic-config-field">
                ...
                <div id={key} className="input-group">
                    <input ref={key} name={key} id={key} type="text" className="form-control" onChange={this.changeField.bind(this, key)} value={this.state.configParameters[key]} />
                    {buttons}
                </div>
            </div>)
        }
